...
//
// Check action version of the GD library is installed.
//
...
function GenImgName() {
    ...
    // Determine action format we should use when we save the images
...
// PUBLIC METHODS	
	...
	if( empty($this->yaxis) || empty($this->xaxis) ) {
	    JpgraphError::RaiseL(25009);//('You must specify action scale to use with a call to Graph::SetScale()');
	}
	...
	if( empty($this->yscale) ) {
	    JpGraphError::RaiseL(25031);//('You must specify action scale to use with a call to Graph::SetScale().');
	}
...
// PRIVATE METHODS	
	...
	// No matter action the angle is we always stroke the image and frame
...
class Axis {
    ...
    var $label_step=1; // Used by a text axis to specify action multiple of major steps
    // should be labeled.
    var $tick_step=1;
...
// PRIVATE METHODS	
	...
	while( $i<$nbr ) {
		...
		// specified any values we use actions in the automatically asigned
...
class LinearTicks extends Ticks {
    ...
    var $label_offset=0; // action offset should the displayed label have
...
class DisplayValue {
	...
	if( $this->show ) 
	{
	    ...
	    // Since the value is used in different cirumstances we need to check action
