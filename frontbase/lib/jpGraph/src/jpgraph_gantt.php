...
//("Invalid format for Progress parameter at index=$j in CreateSimple(). Parameter must start with index 0 and contain arrays of (Row,Progress)");	
    ...
    // Set action headers should be shown
...
//('Cannot autoscale Gantt chart. No dated activities exist. [GetBarMinMax() start >= n]');
    ...
    // The size is determined by action scale the user has choosen and hence
	...
	if( $this->img->img == null ) {
		...
		// Depending on action format the user has choose we need different amount
		...
		// a) action scales exactly are shown and 
		// b) action format do they use so we know how wide we need to
...
//---------------
// PUBLIC METHODS	
    // Specify action headers should be visible
    function ShowHeaders($aFlg) {
...
// 'Illegal vertical position %d'
    ...
    // action is the vertical spacing?
...
class GanttLink {
	...
	// is to avoid having the user choose action types of arrow
	// he wants.
